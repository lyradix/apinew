...
final class PoiController extends AbstractController
{
    ...
    #[Route('/postPoi', name: 'post_poi', methods: ['POST'])]
    public function postPoi(
        ...
        // Validate required fields
        if (!isset($data['popup'], $data['longitude'], $data['latitude'], $data['type'])) {
            return new JsonResponse(['error' => 'Invalid data. "popup", "longitude", "latitude", and "type" are required.'], JsonResponse::HTTP_BAD_REQUEST);
        }
        ...
        try {
            ...
            // Return success response
            return new JsonResponse(['message' => 'Poi inserted successfully.'], JsonResponse::HTTP_CREATED);
        } catch (\Exception $e) {
            // Log the exception details
            error_log('Exception occurred: ' . $e->getMessage());
            error_log('Exception trace: ' . $e->getTraceAsString());
            ...
            return new JsonResponse([
                'error' => 'An error occurred while inserting the Poi.',
                'message' => $e->getMessage()
